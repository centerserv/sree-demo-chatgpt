...
def update_trust(
    ...
    """
    If alpha/beta/gamma/delta are None, perform Optuna tuning for n_trials trials.
    Otherwise run the PPP loop with the provided hyperparameters and return history.
    """
    ...
    def run_trial(trial):
        ...
        for _ in range(iterations):
            V_q = presence.validate(probs)[0] if isinstance(presence.validate(probs), tuple) else presence.validate(probs)
    ...
    # If any param missing, run tuning
